...
static const GUID cfg_timebomb_duration_guid = { 0x8e502ccb, 0xbcfb, 0x4f05, { 0x95, 0x95, 0x3, 0x14, 0xcd, 0x2e, 0x5c, 0xfa } };
...
t_uint32 tb_commands::get_command_count()
{
	return 1;
}

GUID tb_commands::get_command(t_uint32 p_index)
{
	if(p_index == 0)
		return cmd_guid;
...
void tb_commands::get_name(t_uint32 p_index, pfc::string_base& p_out)
{
	if(p_index == 0)
		p_out = "Stop in...";
}

bool tb_commands::get_description(t_uint32 p_index, pfc::string_base& p_out)
{
	if(p_index == 0)
	{
...
private:
	...
	boost::barrier wait_barrier;
	boost::mutex duration_mtx;
...
enum Controls { BTN_STOP=200, BTN_EXIT=300, EDT_DURATION=400 };
...
HWND window_tutorial1::create(HWND p_hWndParent) {
	assert(m_hWnd == NULL);
...
LRESULT WINAPI window_tutorial1::WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
...
LRESULT window_tutorial1::process_message(UINT msg, WPARAM wParam, LPARAM lParam) {
	switch (msg) {

	case WM_CREATE:
		{
			uCreateWindowEx(WS_EX_NOPARENTNOTIFY, "Edit", "", WS_CHILD | WS_VISIBLE | ES_NUMBER, 0, 0, 128, 16, *this, (HMENU)EDT_DURATION, 0, 0);
			uCreateWindowEx(WS_EX_NOPARENTNOTIFY, "Button", "Stop", WS_CHILD | WS_VISIBLE, 0, 16, 64, 16, *this, (HMENU)BTN_STOP, 0, 0);
			uCreateWindowEx(WS_EX_NOPARENTNOTIFY, "Button", "Exit", WS_CHILD | WS_VISIBLE, 64, 16, 64, 16, *this, (HMENU)BTN_EXIT, 0, 0);
	...
	case WM_COMMAND:
		{
			...
			case BTN_STOP:
				{
				boost::thread t(&cb<tb_stop_cb>);
				g_hide_window();
				}
				break;
			case BTN_EXIT:
				{
				boost::thread t(&cb<tb_exit_cb>);
				g_hide_window();
				}
				break;
			}
...
void tb_commands::execute(t_uint32 p_index, service_ptr_t<service_base> foo)
{
	if(p_index == 0)
		g_show_window();
}
...
DECLARE_COMPONENT_VERSION("Timebomb", "0.0.2", "Zao");